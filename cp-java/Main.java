...
public class Main {
    ...
    public static ArrayList<ArrayList<Integer>> connected_components(int n , ArrayList<ArrayList<Integer>> input) {
        //sort the edges based on given input graph;
        ...
        for(int i = 0; i < input.size(); i++) {
            for(int j = 0; j < input.get(i).size(); j++) {
                int u = i;
                int v = input.get(i).get(j);
                adj.get(v).add(u);
            }
        }
 
        //apply the normal dfs;
    ...
    public static void dfs1(int u , ArrayList<ArrayList<Integer>> adj ,int vis[], ArrayList<Integer> res) {
        vis[u] = 1;
        res.add(u);
        for(int child : adj.get(u)) {
            if(vis[child] == 0) {
                dfs1(child, adj, vis, res);
            }
        }
    }
 
    public static void dfs(int u , ArrayList<ArrayList<Integer>> adj , Stack<Integer> st, int vis[]) {
        vis[u] = 1;
        for(int child : adj.get(u)) {
            if(vis[child] == 0) {
